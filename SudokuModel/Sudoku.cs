...
namespace SudokuModel
{
	public class Sudoku
	{
        ...
        public byte[,] read_from_file()
        {

            byte[,] sudo = new byte[9, 9];

            string[] lines = System.IO.File.ReadAllLines(@"F:\New folder\sudoko#1.txt");


            for (int line = 0; line < lines.Length; line++)
            {
                char[] words = lines[line].ToCharArray();

                for (int k = 0; k < words.Length; k++)
                {
                    if (words[k] != '*')
                    {
                        sudo[line, k] = (byte)char.GetNumericValue(words[k]);
